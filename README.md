...
- Roll (φ), Pitch (θ), and Yaw (ψ) sliders with live numeric readouts
- Real-time 3×3 rotation matrix display (updates on every slider movement)
- ZYX Euler-angle rotation convention: `R = Rz(ψ) · Ry(θ) · Rx(φ)`
- Reset button that returns the system to the identity rotation
...
- Legend and on-canvas HUD showing current angle values and frame status
